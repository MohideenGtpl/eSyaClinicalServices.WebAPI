            ...
            try
            {
                using (var db = new eSyaEnterprise())
                {
                    using (var dbContext = db.Database.BeginTransaction())
                    {
                        try
                        {
                            var isSpecialtyExist = db.GtEsspcd.Where(x => x.SpecialtyDesc.ToUpper().Trim() == obj.SpecialtyDesc.ToUpper().Trim()).Count();
                            if (isSpecialtyExist > 0)
                            {
                                return new DO_ReturnParameter() { Status = false, Message = "Specialty already Exists" };
                            }
